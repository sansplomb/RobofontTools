...
class Process(BaseWindowController):
	...
	def showProgress(self, sender):
		if self.w.textBoxEncodingL.get():
			...
			self.progress.close()
			self.progress = self.startProgress("Processing L...", tickCount=100)
			self.makeGlyphs(f, glyphsToMake)
			self.progress.close()
		elif self.w.textBoxEncodingM.get():
			self.progress = self.startProgress("Checking UFO...", tickCount=100)
			glyphsToMake = self.defineGlyphsToMake(f, extrasM)
			self.progress.close()
			self.progress = self.startProgress("Processing M...", tickCount=100)
			self.makeGlyphs(f, glyphsToMake)
			self.progress.close()
		elif self.w.textBoxEncodingS.get():
			self.progress = self.startProgress("Checking UFO...", tickCount=100)
			glyphsToMake = self.defineGlyphsToMake(f, extrasS)
			self.progress.close()
			self.progress = self.startProgress("Processing S...", tickCount=100)
			self.makeGlyphs(f, glyphsToMake)
			self.progress.close()
		self.w.close()
